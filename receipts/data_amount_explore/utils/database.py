    ...
    def compute_stats(self):
        ...
        for file_id, ema, wav in self.data:                        
            if idx == 0:
                ...
                wav_all = np.concatenate((wav_all, wav), axis = 0)
            idx += 1                        
        ...
        ema_min, ema_max = np.min(ema_all, axis = 0), np.max(ema_all, axis = 0)
        wav_min, wav_max = np.min(wav_all, axis = 0), np.max(wav_all, axis = 0)
        return ema_mean, ema_std, wav_mean, wav_std, ema_min, ema_max, wav_min, wav_max
